...
bool DatFile::Load(const char *filename, ID3D11Device* device11, ID3D11DeviceContext*	device11Context)
{
	...
	sts = CreateConstantBuffer(
		device11,
		sizeof(X::ConstantBufferColor),
		&m_cb5);

	if (!sts) {
		MessageBox(NULL, "CreateBuffer(constant buffer Color) error", "Error", MB_OK);
		return false;
	}
	// シェーダーリソースビュー作成
	...
	for (int i = 0; i < m_datfile->m_MaterialSuu; i++){
		if (m_datfile->m_DecalTex[i] != "none" && m_datfile->m_DecalTex[i] != "null"){
			// テクスチャ読み込み
...
void DatFile::SetColorMaterial(XMFLOAT4 setColor,ID3D11DeviceContext* device) {
	//カラーを修正する
	//Drawの前に呼び出す
	X::ConstantBufferColor cb;

	cb.ColorMaterial.x = setColor.x;
	cb.ColorMaterial.y = setColor.y;
	cb.ColorMaterial.z = setColor.z;
	cb.ColorMaterial.w = setColor.w;
