...
protected:
	...
	struct ConstantBufferViewPort {
		XMFLOAT4 ViewportWidth;
		XMFLOAT4 ViewportHeight;
	};
...
class Draw2D :public Draw2DBase {
private:
	...
	float   m_sy = 1.0f;
	float   m_sz = 1.0f;
	VertexDetail				m_Vertex[4];		// 頂点座標＊４
...
public:
	void ChangePosSize(float x, float y, float z, float width, float height) {
		...
		if (SUCCEEDED(hr)) {
			...
			memcpy_s(pData.pData, pData.RowPitch, (void*)(m_Vertex), sizeof(VertexDetail) * 4);
	...
	void Draw() {
		...
		// 頂点バッファをセットする
		unsigned int stride = sizeof(VertexDetail);
		unsigned  offset = 0;
	...
	bool Init(float x, float y, float z, float width, float height, XMFLOAT4 color, const char* filename) {
		...
		sts = CreateVertexBufferWrite(
			m_dev,
			sizeof(VertexDetail),			// １頂点当たりバイト数
			4,						// 頂点数
